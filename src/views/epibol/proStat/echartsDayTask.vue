...
<script>
  export default{
      ...
      methods: {
          dataGet(){
              let _this=this;
              let url=this.GLOBAL.baseRouter+this.$GLOBALAPI.task_total_status_by_day+"&project_id="+this.projectID;
              _this.$axios.get(url).then(msg=>{
                  ...
                  if (msgData.err_code==0){
                      ...
                      if (msgData.data.series!=undefined){
                          let dateName=msgData.data.series.map((val)=>({"name": val.name}));
          ...
          myCharts(){
              ...
              var myChart = echarts.init(document.getElementById('echartLine'), 'ehcartTheme');
        // 绘制图表
              myChart.setOption({
                  tooltip: {
                      trigger: 'item',
