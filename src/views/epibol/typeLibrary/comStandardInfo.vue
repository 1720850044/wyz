<template>
    <Content class="taskClassLibrary">
        <div class="taskClassLiHeader">
                    ...
                    <div v-if="!disabled" v-show="boxShow" class="taskTypeNameBox">
                        <div v-for="(item,index) in TaskIdentity" :key="index" class="standard-label">
                            <img :src="item.url" @click="secMarkers(item,'icon')"
                                 :class="{'iconColor':identification.iconBorder === item.id}" class="identification"
                                 width="30" height="36">
            ...
            <template v-else v-if="!disabled">{{$t('companySpecificationLibrarys.com_identificationIcon')}}</template>
          </span>
                </section>
                <Input :placeholder="$t('companySpecificationLibrarys.com_newSpecification')" v-model="typename.typename" style="" :disabled="disabled"></Input>
            ...
            <div v-if="isSubmit||project">
                ...
                <!--<Button v-if="!project&&!temporary" @click="moveStandard" type="primary" class='standardOperateButton'>移动</Button>-->
                <!--临时隐藏-->
                <Button id="librarySaveButton" @click="submitTaskClas" type="primary" :disabled="saveDis"
                        class='standardOperateButton'>{{$t('lang.lang_save')}}
                </Button>
            </div>
                ...
                <Col class="taskclass-guide-main" span="16">
                    ...
                    <div class="stepCentral">
                        <Steps v-if="fstandard" :current="current" direction="vertical" size="small">
                            <Step v-for="(step,index) in fstandard"
                                  :id="index === 0 ? 'stepContainer' : index === fstandard.length-1 ? 'stepContainerLa':''"
                                  class="stepContainer" :key="index">
                                <div class="stepRight">
                                    <!--步骤名称-->
                                    <div class="stepRightBox">
                                        ...
                                        <input v-model="step.stage_name" :disabled="disabled" class="title" type="text"
                                               prefix=''/>
                                        ...
                                        <div v-if="project" :id="index === 0 ? 'inSideSwitch' : ''">
                                            <Checkbox v-model="step.is_inside_audit" :true-value="1" :false-value="0"
                                                      :key="index">{{$t('project.pro_internalAudit')}}
                                            </Checkbox>
                                            <Checkbox v-if="collaboration || is_collaboration"
                                                      v-model="step.is_client_audit" :true-value="1" :false-value="0"
                                                      :key="index">{{$t('project.pro_externalAudit')}}
                                            </Checkbox>
                                        </div>
                                        <p class="acmSign" v-if="fstandard.length === index+1">{{$t('project.pro_deliveryPeriod')}}</p>
                                        <Icon class="deleteStepIcon" v-if="!disabled"
                                              @click.native="deleteStep(index,step)" type="close-circled"></Icon>
                                    </div>
                                    <!---->
                                    <div>
                                        <ul class="stepsUl">
                                            <li class="stepsList" v-for="(list,i) in step.require" :key="i">
                                                ...
                                                <Input v-model="list.text" :placeholder="$t('project.pro_enterStageClaim')" :disabled="disabled"
                                                       class="borderBottomSty"></Input>
                                                <div class="priorityContainer">
                                                    <p v-if="list.level !== '0'" class="priority priorityExhibition"
                                                       :class="`priority${list.level}`">
                                                        {{list.level | priorityValue}}</p>
                                                    <div v-if="!disabled" class="priorityList">
                                                        <p class="priority priority3"
                                                           @click="editPriority('3',index,i)">{{$t('lang.lang_height')}}</p>
                                                        <p class="priority priority2"
                                                           @click="editPriority('2',index,i)">{{$t('lang.lang_mid')}}</p>
                                                        <p class="priority priority1"
                                                           @click="editPriority('1',index,i)">{{$t('lang.lang_low')}}</p>
                                                    </div>
                                                </div>
                                                <!--规范列表-->
                                                <Taskselect :isSubmit="isSubmit" :project="project" :parentIndex="index"
                                                            :chilenIndex="i" :module="fstandard"
                                                            :normName="list.norm_name" :dataList="norms"
                                                            :disabled="disabled" v-on:Selectdata="updataSelect"
                                                            v-on:addNormsFun="addNorms"
                                                            :callbackStatus="callbackStatus"></Taskselect>
                                                <Icon v-if="!disabled" type="trash-a" class="delIcon"
                                        ...
                                        <!--流程info增加-->
                                        <div class="stepsUl">
                                            <Icon v-show="!step.show" v-if="!disabled" class="addSteps"
                                                  @click.native="stepAdd(index)" type="ios-plus-outline"
                                                  style="margin-left:5px;" :disabled="disabled"></Icon>
                                        </div>
                                    </div>
                                </div>
                            </Step>
                        ...
                        <!--<div>-->
                        <Icon id="addSteps" @click.native="addSteps" v-if="!disabled"
                        ...
                        <ul>
                            ...
                            <li>
                                ...
                                <p class="titleFloat">
                                    ...
                                    <Icon v-if="!fileShow&& !disabled" class="addSteps" @click="addFileat"
                                ...
                                <template v-if="pstandard && pstandard.length>0">
                                    <Row v-for="(item,index) in pstandard" :key="'pstandard'+index" class="fileAttr">
                                        <Col span="8">
                                            <AutoComplete class='standardName' v-model="item.name" :placeholder="$t('project.pro_normativeName')"
                                                          :clearable="!disabled" :disabled="disabled">
                                            </AutoComplete>
                                        </Col>
                                        <Col span="16">
                                            <AutoComplete class='standardContent' v-model="item.values"
                                                          :placeholder="$t('project.pro_normativeDescribe')" :clearable="!disabled"
                                                          :disabled="disabled">
                                            </AutoComplete>
                                        </Col>
                                        <Icon v-if="!disabled" @click.native="removeFileat(index)" type="trash-a"
                            ...
                            <li>
                                ...
                                <p class="titleFloat">
                                    ...
                                    <Icon v-if="!OtherfileShow&&!disabled" class="addSteps" @click="addJunctShow"
                                          type="ios-plus-outline"></Icon>
                                </p>
                                <template v-if="tstandard && tstandard.length>0">
                                    <div v-for="(item,index) in tstandard" :key="index" class="attachingTask"
                                         style="display: flex">
                                        <Col span="8">
                                            <AutoComplete class='standardName' v-model="item.name" :placeholder="$t('project.pro_normativeName')"
                                                          :clearable="!disabled" :disabled="disabled"></AutoComplete>
                                        </Col>
                                        <Col span="16">
                                            <AutoComplete class='standardContent' v-model="item.values"
                                                          :placeholder="$t('project.pro_normativeDescribe')" :clearable="!disabled"
                                        ...
                                        <Icon v-if="!disabled" @click.native="delOtherfile(index)" type="trash-a"
                                              class="delIcon" :disabled="disabled"></Icon>
                                    </div>
                                </template>
                            </li>
