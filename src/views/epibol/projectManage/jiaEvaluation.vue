    ...
    <div class="scoreContent">
        <div class="filterBar">
            <Select v-model="creentStatus" style="width:100px;margin-right: 10px" @on-change="selectFun">
                <Option v-for="(item,index) in selectionData" :value="item.status" :key="index">{{ item.value }}
                </Option>
            </Select>
            <Input v-model="searchText" style="width:200px" icon="ios-search" :placeholder="inputProjectName"
                   @keyup.enter.native="searchfun" class="search-input"></Input>
        </div>
        <div class="contenerList">
            ...
            <ul class="listSty" v-if="!showLoading && listArr && listArr.length > 0">
                <li v-for="(item,index) in listArr" :key="index">
                    <span><i class="evaluate"
                             @click="hrefPage('project',item.project_id)">{{item.project_name}}</i></span>
                    ...
                    <span>{{item.time}}</span>
                    <span><i class="evaluate"
                             @click="showModal(item)">{{item.score_status == 0 ? toEvaluate : viewEvaluation}}</i></span>
                </li>
            </ul>
        ...
        <Modal v-model="modelFlag" :footer-hide="footerFlag" :title="projectScore" width="900">
            <div class="modalContener">
                <div class="top">
                    ...
                    <div class="right">
                        <ul v-if="bannerArr && bannerArr.length > 0" :style="{width:bannerArr.length * 520 + 'px'}"
                            id="bannerUl" ref="bannerUl">
                        ...
                        <div v-if="bannerArr && bannerArr.length > 0" class="pageTool">
                            <em class="iconfonts icon-fangxiangshang leftsty" @click="prev"></em> <span>{{bannerObj.index}} / {{bannerArr.length}}</span>
                            <em class="iconfonts icon-fangxiangshang rightsty" @click="next"></em>
                        </div>
                ...
                <div class="bottom">
                    ...
                    <div class="star">
                        <div class="left">
                            <ul>
                                <li>
                                    ...
                                    <Rate style="font-size: 15px" v-model="projectInfos.make_score"
                                          :disabled="disabledFlag"/>
                                    <span>{{projectInfos.make_score | convert}}</span>
                                </li>
                                <li>
                                    <span>{{$t('resources.res_linkUp')}}</span>
                                    <Rate style="font-size: 15px" v-model="projectInfos.communication_score"
                                          :disabled="disabledFlag"/>
                                    <span>{{projectInfos.communication_score | convert}}</span>
                                </li>
                                <li>
                                    <span>{{$t('resources.res_manuscriptQuality')}}</span>
                                    <Rate style="font-size: 15px" v-model="projectInfos.quality_score"
                                          :disabled="disabledFlag"/>
                        ...
                        <div class="right">
                            <Input style="font-size: 12px" v-model="projectInfos.content" :disabled="disabledFlag"
                                   :autosize="{minRows: 5,maxRows: 5}" :maxlength="500" type="textarea"
                                   :placeholder="inputEvaluationContent"/>
                            <label class="enter-desc-counts"><i
                                :class="{'errorSty':enterNum > 500}">{{enterNum}}</i><span> / </span><i>500</i></label>
                        </div>
                    </div>
            ...
            <div slot="footer">
                <button class="confimBtn" @click="confirmFun">{{$t('resources.res_confirmRelease')}}</button>
            </div>
        </Modal>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                showLoading: true,
                toEvaluate: i18n.t('resources.res_toEvaluate'),
                inputEvaluationContent: i18n.t('resources.res_inputEvaluationContent'),
                ...
                viewEvaluation: i18n.t('resources.res_viewEvaluation'),
        ...
        methods: {
            //初始化接口
            async init() {
                let {data} = await scoreApi.scoreInit();
                if (data.err_code == 0) {
                    this.getProjectList();
                } else {
            ...
            async getProjectList() {
                ...
                let {data} = await scoreApi.getProjectHttp(parms);
                if (data.err_code == 0) {
            ...
            async showModal(item) {
                ...
                let {data} = await scoreApi.getProjectDtailHttp({project_id: item.project_id});
                if (data.err_code == 0) {
                    this.resetBanner();
            ...
            //提交评价
            async confirmFun() {
                let projectInfos = this.projectInfos;
                let enterNum = this.enterNum;
                if (projectInfos.make_score == 0 || projectInfos.communication_score == 0 || projectInfos.quality_score == 0) {
                    this.$Message.error(i18n.t('resources.res_pleaseGrade'));
                } else if (enterNum == 0) {
                    this.$Message.error(i18n.t('resources.res_evaluationContent'));
                } else if (enterNum > 500) {
                    this.$Message.error(i18n.t('resources.res_commentaryText'));
                } else {
                    let parms = {
                        project_id: projectInfos.project_id,
                        make_score: projectInfos.make_score,
                        communication_score: projectInfos.communication_score,
                        quality_score: projectInfos.quality_score,
                        content: projectInfos.content
                    };
                    let {data} = await scoreApi.ReleaseHttp(parms);
                    if (data.err_code == 0) {
                        this.disabledFlag = true;
                        this.footerFlag = true;
                        this.$Message.success(i18n.t('resources.res_evaluationSuccess'));
                        this.getProjectList();
                    } else {
                        this.$Message.error(data.err_message);
                    }
                ...
                if (id) {
                    switch (type) {
                        ...
                        case 'contract' :
                            ...
                            break;
                        case 'testTask' :
                            this.$router.push({path: '/customer/testTaksDetails', query: {id: id}});
                            break;
                    }
                }
        ...
        filters: {
            convert(number) {
                ...
                switch (number) {
                    case 0 :
                        str = i18n.t('resources.res_pleaseGrade');
                        break;
                    case 1 :
                        str = i18n.t('resources.res_range');
                        break;
                    case 2 :
                        str = i18n.t('resources.res_commonly');
                        break;
                    case 3 :
                        str = i18n.t('resources.res_bad');
                        break;
                    case 4 :
                        str = i18n.t('resources.res_satisfied');
                        break;
                    case 5 :
                        str = i18n.t('resources.res_verySatisfied');
                        break;
                }
                return str;
            }
