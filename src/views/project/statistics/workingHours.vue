        ...
        <Col span="24" class-name='bgw'>
            <div class="itemBox base-card">
              <div class="base-card-head">
                <div class="base-card-operation-head" >成员工作用时</div>
                <div class="base-card-operation-icon" ><!--p class="iconfonts icon-shenglve-"></p--></div>
                <div style="clear:both"></div>
              </div>
                <worke-echars v-if="MemberObjData.data"
                    :objData="MemberObjData" 
                    :accToken="powerToken"
                    :censusType = "'bar'"
                    :echarsId = "'taskRatework'"
                    :stack = "true"
                    :yAxis = "yAxis"
                    :isRoll = "false"
                    :percentage = "'task_rate'"
                    :dataZoom = "dataZoom"
                    :dataZoomRoll = "dataZoomRoll"
                    :formatter = "'workingHours'"
...
<script>
import WorkeEchars from "../../../components/WorkeEchars.vue";
...
export default {
  ...
  data() {
    return {
      ...
      yAxis: [
          {name: '工时',type: 'value',min: 0},
          {name: '工时利用率',type: 'value',min: 0,
              axisLabel: {
                  formatter: '{value}%'
              }
          }
  ...
  methods: {
    ...
    getMemberInfo () {
        var self = this;
        this.MemberEcharsUrl =this.GLOBAL.baseRouter + onlyAPI.project_total_work_time;
