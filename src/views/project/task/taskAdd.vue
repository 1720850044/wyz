    ...
    <div class="taskDetailsContainer" :class="{'martop':type !== 'newTask'}">
        <Form :label-position="'left'" :label-width="70">
            <FormItem label="任务名称">
                ...
                <Input v-model="taskInfos.name" :disabled="companyType && taskInfos.is_collaboration ? true : type == 'taskDetail' && Boolean(!detailRole.title)" @on-blur="editTaskAutoSave(true)" placeholder="请输入任务名称"></Input>
            </FormItem>
            ...
            <FormItem label="制作人" class="borBotm">
                <i class="requiredIcon" v-if="type == 'newTask'">*</i>
                <bby-select :v-model="taskInfos.remark_name" class="bbySelect" ref="bbySelectMember" placeholder="请选择制作人" :selectTab="memberSelectTab" :disabledFlag="companyType && taskInfos.is_collaboration ? true : type == 'taskDetail' && Boolean(!detailRole.run_user)">
                    <!--项目-->
                    ...
                    <template slot="select2" v-if="memberDataCom">
                        <Input v-model="searchVal" placeholder="人员搜索" @on-keyup="searchMember"  icon="ios-search"  style="width: calc(100% - 20px);margin: 0 10px 0 10px;"></Input>
            ...
            <FormItem label="任务类型" class="borBotm">
                <i class="requiredIcon" v-if="type == 'newTask'">*</i>
                <bby-select :v-model="taskInfos.tasktype_name"   ref="bbySelectTask" placeholder="请选择任务类型" :selectTab="standardSelectTab" :disabledFlag="companyType && taskInfos.is_collaboration ? true : type == 'taskDetail' && Boolean(!detailRole.task_type) ">
            ...
            <!--<FormItem label="任务类型" console.log()lass="borBotm">-->
                ...
                <!--</Select>-->
            <!--</FormItem>-->
            <FormItem label="任务时间" class="borBotm dateBorBotm">
                <i class="requiredIcon" v-if="type == 'newTask'">*</i>
                <!--{{getTimeRange}}-->
                <!--format="yyyy-MM-dd HH:mm:ss"-->
                <DatePicker class="taskdatepicker" format="yyyy-MM-dd" type="daterange"  :value="getTimeRange" :readonly="companyType && taskInfos.is_collaboration ? true : detailRole.flag ? Boolean(!detailRole.run_time) : false" @on-change="setTimeRange"  split-panels style="width: 100%" placeholder="选择任务开始和结束时间"></DatePicker>
            </FormItem>
            <FormItem label="子项目" class="borBotm">
                <Select v-if="taskInfos.project_child != 'NULL'" v-model="taskInfos.project_child" :disabled="companyType && taskInfos.is_collaboration ? true : detailRole.flag ? Boolean(!detailRole.project_child) : false" @on-change='editTaskAutoSave(false)' placeholder="请选择子项目">
            ...
            <FormItem label="参考附件" class="borBotm">
                        ...
                        <template slot="upload">
                            ...
                            <ul style="max-height: 140px;overflow: auto">
                                <li class="flieList" v-for="(items,index) in taskInfos.file" :key="index">
                                    ...
                                    <span>
                                        <a :href="items.url+'?response-content-type=application%2Foctet-stream'" download="" style="color: #657180">下载 &nbsp;</a>
                ...
                <p v-else style="height: 40px;line-height: 40px;">暂无</p>
            </FormItem>
    ...
    export default {
        ...
        data() {
            return {
                ...
                memberProjectI: null,
                memberData: null,
                standardSelectTab: ['项目规范', '公司通用规范'],
                memberSelectTab: ['项目', '公司'],
                data5: [
                    {
                        title: 'parent 1',
                        expand: true,
        ...
        methods: {
            ...
            async getProTaskType() {
                ...
                let {data} = await pjTaskList({project_id: this.projectId, });
                if (data.err_code === 0 && data.data) {
            ...
            * */
            ...
            async getjoinMeberList(){
                ...
                let {data} = await pjTaskJoined({project_id: this.projectId, needGuideData: this.needGuideData});
                if (data.err_code == 0){
            ...
            async getTaskTypeList(){
                ...
                let {data} = await pjTaskList({project_id: this.projectId, needGuideData: this.needGuideData});
                if (data.err_code == 0){
            ...
            async getChildProjectList(){
                ...
                let {data} = await pjTaskChildList({project_id: this.projectId, needGuideData: this.needGuideData});
                if (data.err_code == 0){
            ...
            async updateTaskDetail(dataForm, type) {
                ...
                if (data.err_code == 0){
                    ...
                    if (type === 'childTask'){
                        this.$Message.error("子任务编辑失败，请重试！");
