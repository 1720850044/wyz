...
function transformToPrintable(data){
    for (var prop in data) {
        var value = data[prop];
        if (typeof(value)==='string') {data[prop]=(value + '').replace(/\n/g, '<br>');}
    }
    return data;
}
...
function isValidCurrency(numStr){
  //first try to convert format in a regex
    ...
    if (!minusFound) {regex='[-]?'+regex;}

    if (currencyString!='') {regex=regex+'(?:'+RegExp.quote(currencyString)+')?';}
...
function getCurrencyValue(numStr){
    if (!isValidCurrency(numStr)) {return NaN;}
...
function formatNumber(numberString, format) {
  ...
  // we can have formats like -$23.32
    ...
    for (var i = format.length - 1; i >= 0; i--) {
        if (validFormat.indexOf(format.charAt(i)) == -1) {suffix = format.charAt(i) + suffix;} else {break;}
    }
  ...
  // special case for percentages
    ...
    // round or truncate number as needed
        if (round) {number = new Number(number.toFixed(decimalFormat.length));} else {
    ...
    var onesFormat = '';
    if (format.indexOf('.') == -1) {onesFormat = format;} else {onesFormat = format.substring(0, format.indexOf('.'));}
...
function millisFromHourMinute(stringHourMinutes) { //All this format are valid: "12:58" "13.75"  "63635676000" (this is already in milliseconds)
    ...
    if (semiColSeparator < 0 && dotSeparator < 0 && stringHourMinutes.length > 5) {
        ...
        if (dotSeparator > -1) {
            ...
            var hour = 0;
            var minute = 0;
            if (semiColSeparator == -1) {hour = parseInt(stringHourMinutes, 10);} else {
...
function Profiler(name) {
    ...
    this.stop = function() {
        if (!__profiler[this.name]) {__profiler[this.name] = {millis: 0, count: 0};}
...
function openBlackPopup(url, width, height, onCloseCallBack, iframeId){
    if (!iframeId) {iframeId='bwinPopup';}
  ...
  //add black only if not already in blackpupup
    if (window.name!=iframeId) {bg.css({backgroundImage: "url('"+contextPath+"/applications/teamwork/images/black_70.png')"});}
