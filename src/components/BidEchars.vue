  ...
  name: "CompanyEchars",
    ...
    methods: {
        ...
        columnsR (key) { // table控制行列表项
            ...
            if(this.formatter == 'cooperation'){
                this.dataTab.forEach((val,index) => {
                    val.price = String(val.price).replace(/(?=(\B)(\d{3})+$)/g,',');
                })
            }
        ...
        echarsRender () {
            ...
            this.$PublicFuns.drawEchart(()=>{
                ...
                myChart.setOption({
                    ...
                    tooltip: {
                        trigger: "axis", //this.formatter
                        formatter:function(param){
                            var relVal = param[0].name;
                            for(var i=0;i<param.length;i++){
                                if(self.formatter == 'cooperation'){
                                    if(i == 1){
