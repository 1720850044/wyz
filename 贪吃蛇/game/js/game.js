...
//     }
//     static __init__ () {
//         let checkpoint = document.getElementsByClassName('checkpoint')[0].innerHTML,
//             Jgametpl = document.getElementsByClassName('J_game_tpl')[0].innerHTML,
//             Jtitle = document.getElementsByClassName('J_title')[0].innerHTML,
//             frag = document.createDocumentFragment();
//         oBox.innerHTML += Jtitle;
//     }
// }
            ...
            doc.timer = setInterval(function(){
                ...
                function m(){
                    ...
                    if(getStyles(oEnemy,'top') >= boxH){
                        ...
                        var allBiu = doc.getElementsByClassName('biu'),
                            len = allBiu.length;
                        for(var i = 0;i<len;i++){
                            if(coll(oEnemy,allBiu[i])){
                                cancelAnimationFrame(allBiu[i].timer);
                                doc.removeChild(allBiu[i]);
                                createBoom(oEnemy , "",doc);
                                //移除敌军
                                doc.removeChild(oEnemy);
                                return
                            }
                         ...
                         if ( oPlane.parentNode && coll( oEnemy , oPlane ) ){
                            ...
                            clearInterval(doc.timer); //停止敌军生成
                            //生成爆炸图片
                            createBoom(oEnemy , "",doc);
                            createBoom(oPlane , "2",doc);
                            //移除敌军/我军
                            doc.removeChild(oEnemy);
                            doc.removeChild(oPlane);
                            //解除鼠标移动事件
